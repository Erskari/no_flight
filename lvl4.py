...
def generate_commands(game_state):
    ...
    if len(game_tick_count) == 1:
        # Pythagoraan lauseellaa saadaan koneille lähestymiskulmat
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        new_dir = normalize_heading(aircraft[1]['direction'] + 15)
        commands.append(f"HEAD {aircraft[1]['id']} {new_dir}")
        return commands

    # Loput if lauseista kääntää koneita 90° - lähestymiskulma mahdollisimman nopeasti
    elif len(game_tick_count) == 37:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 38:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 39:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 40:
        new_dir = normalize_heading(aircraft[0]['direction'] + 10)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 47:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 48:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 49:
        new_dir = normalize_heading(aircraft[0]['direction'] + 20)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    elif len(game_tick_count) == 50:
        new_dir = normalize_heading(aircraft[0]['direction'] + 15)
        commands.append(f"HEAD {aircraft[0]['id']} {new_dir}")
        return commands
    else:
        return commands
